...
/*   Updated: 2025/11/03 20:02:01 by cochatel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_v_port(t_scene *scene)
{
	...
	camera = &scene->camera;
	viewport->v_port_height = 2.0f * tan(to_radian(camera->fov) \
			/ 2.0f) * V_PORT_DIST;
	viewport->v_port_width = WIN_WIDTH * viewport->v_port_height / WIN_HEIGHT;
	viewport->v_port_center = add_vector(camera->pos, \
			scale_vector(camera->forward, V_PORT_DIST));
