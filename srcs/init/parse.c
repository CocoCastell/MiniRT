...
/*   Updated: 2025/11/08 13:30:56 by cochatel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_ambient(t_minirt *minirt, char **data)
{
	...
	if (!ft_is_str_float(data[1]))
		free_error(minirt, "Ambient ratio: invalid format.\n", 1);
...
void	parse_camera(t_minirt *minirt, char **data, int fov)
{
	...
	if (!ft_is_str_float(data[3]))
		free_error(minirt, "FOV: invalid format.\n", 1);
	fov = ft_atoi(data[3]);
	if (fov < 0 || fov > 180)
...
void	parse_light(t_minirt *minirt, char **data)
{
	...
	if (!ft_is_str_float(data[2]))
		free_error(minirt, "Intensity: invalid format.\n", 1);
...
int	put_object_in_structure(char *line, t_minirt *minirt)
{
	...
	if (obj_data[0][0] == 'C')
		parse_camera(minirt, obj_data, 0);
