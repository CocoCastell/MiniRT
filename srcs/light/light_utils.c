 ...
 * @param max_dist Max distance to consider for intersections (dist to light).
...
bool	pre_shadow_calcul(t_hit_info *hit, t_ray ray)
{
	if (hit->type == SPHERE && dot(hit->normal, ray.direction) <= 0)
		return (true);
	if (hit->type == PLANE && ((dot(hit->normal, ray.direction) < 0 && \
		dot(hit->normal, hit->incident_ray) < 0) || (dot(hit->normal, \
		ray.direction) > 0 && dot(hit->normal, hit->incident_ray) > 0)))
		return (true);
	return (false);
}
...
float	distance_attenuation(t_vec3 vector)
{
	...
	b = 0.01f;
	c = 0.00025f;
	return (1.0f / (a + b * vector_length(vector) + c * \
		vector_sq_length(vector)));
}
